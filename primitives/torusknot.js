function TorusKnot(epicCanvas, radius = 1.0, tubeRadius = 0.3, pathSegments = 60, tubeSegments = 16, p = 2, q = 3, initBuffers = true) {
    const vertices = [];
    const colors = [];
    ...
    for (let i = 0; i <= pathSegments; i++) {
        const t = (i / pathSegments) * 2.0 * Math.PI * q; // Scale by q for knot periodicity

        // Torus knot parametric equations
        const phi = t;
        const x = radius * Math.cos(p * phi) * (1 + 0.5 * Math.cos(q * phi));
        const y = radius * Math.sin(p * phi) * (1 + 0.5 * Math.cos(q * phi));
        const z = radius * 0.5 * Math.sin(q * phi);

        // Derivatives for tangent (approximated numerically)
        const delta = 0.001;
        const x2 = radius * Math.cos(p * (phi + delta)) * (1 + 0.5 * Math.cos(q * (phi + delta)));
        const y2 = radius * Math.sin(p * (phi + delta)) * (1 + 0.5 * Math.cos(q * (phi + delta)));
        const z2 = radius * 0.5 * Math.sin(q * (phi + delta));
        const tangent = normalize([(x2 - x) / delta, (y2 - y) / delta, (z2 - z) / delta]);
        ...
        for (let j = 0; j <= tubeSegments; j++) {
            ...
            colors.push(0.5 + 0.5 * Math.cos(p * phi), 0.5 + 0.5 * Math.sin(q * phi), 0.5, 1.0);
        }
    }
